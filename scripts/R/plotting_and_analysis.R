...
if (replot_raw_plots){
  ...
  cat("  → Generating Brier Skill Score plot...\n")
  plot_results(
  ...
  cat("  → Generating raw Brier Score plot...\n")
  plot_results(
    post_training_data_summarised,
    y     = "Brier Score",
    colour  = "recurrence",
  ...
  cat("  → Generating inverse Brier Score plot...\n")
  plot_results(
    post_training_data_summarised,
  ...
  cat("  → Generating proportion correct plot...\n")
  plot_results(
    post_training_data_summarised,
  ...
  cat("Filtering data for specific input sizes (no finite-state solutions)...\n")
  ...
  cat("  → Generating filtered Brier Skill Score plot...\n")
  plot_results(
  ...
  cat("  → Generating filtered raw Brier Score plot...\n")
  plot_results(
    post_training_data_summarised_specific_inputs,
    y     = "Brier Score",
    colour  = "recurrence",
  ...
  cat("  → Generating filtered inverse Brier Score plot...\n")
  plot_results(
    post_training_data_summarised_specific_inputs,
    y     = "Inverse Brier Score",
    colour  = "recurrence",
  ...
  cat("  → Generating filtered proportion correct plot...\n")
  plot_results(
    post_training_data_summarised_specific_inputs,
    y     = "Proportion Correct",
    colour  = "recurrence",
  ...
  cat("  → Brier Skill Score by input size...\n")
  plot_results(
  ...
  cat("  → Raw Brier Score by input size...\n")
  plot_results(
    dplyr::filter(post_training_data_summarised, recurrence != "FFN"),
    y     = "Brier Score",
    colour  = "inputsize",
  ...
  cat("  → Inverse Brier Score by input size...\n")
  plot_results(
    dplyr::filter(post_training_data_summarised, recurrence != "FFN"),
    y     = "Inverse Brier Score",
    colour  = "inputsize",
  ...
  cat("  → Proportion Correct by input size...\n")
  plot_results(
    dplyr::filter(post_training_data_summarised, recurrence != "FFN"),
    y     = "Proportion Correct",
    colour  = "inputsize",
  ...
  cat("  → Input size path plot (Brier Skill Score)...\n")
  plot_results_path(
    dplyr::filter(post_training_data_summarised, recurrence != "FFN"),
    x     = "inputsize",
    y     = "Brier Skill Score",
  ...
  cat("Generating path plots...\n")

  cat("  → Input size by grammar (Brier Skill Score)...\n")
  plot_results_path(
    dplyr::filter(post_training_data_summarised, recurrence != "FFN"),
    x     = "inputsize",
    y     = "Brier Skill Score",
  ...
  cat("  → Input size by grammar (raw Brier Score)...\n")
  plot_results_path(
    dplyr::filter(post_training_data_summarised, recurrence != "FFN"),
    x     = "inputsize",
    y     = "Brier Score",
  ...
  cat("  → Input size by grammar (inverse Brier Score)...\n")
  plot_results_path(
    dplyr::filter(post_training_data_summarised, recurrence != "FFN"),
    x     = "inputsize",
    y     = "Inverse Brier Score",
  ...
  cat("  → Input size by grammar (proportion correct)...\n")
  plot_results_path(
    dplyr::filter(post_training_data_summarised, recurrence != "FFN"),
    x     = "inputsize",
    y     = "Proportion Correct",
  ...
  cat("  → Brier Skill Score by neurons...\n")
  plot_results(
  ...
  cat("  → Raw Brier Score by neurons...\n")
  plot_results(
    dplyr::mutate(post_training_data_summarised, 
                  neurons = factor(neurons, levels = seq(32, 512, 32))),
    y     = "Brier Score",
  ...
  cat("  → Inverse Brier Score by neurons...\n")
  plot_results(
    dplyr::mutate(post_training_data_summarised, 
                  neurons = factor(neurons, levels = seq(32, 512, 32))),
    y     = "Inverse Brier Score",
  ...
  cat("  → Proportion Correct by neurons...\n")
  plot_results(
    dplyr::mutate(post_training_data_summarised, 
                  neurons = factor(neurons, levels = seq(32, 512, 32))),
    y     = "Proportion Correct",
  ...
  cat("  → Brier Skill Score by laminations...\n")
  plot_results(
    post_training_data_summarised,
    y     = "Brier Skill Score",
    colour  = "laminations",
  ...
  cat("  → Raw Brier Score by laminations...\n")
  plot_results(
    post_training_data_summarised,
    y     = "Brier Score",
    colour  = "laminations",
  ...
  cat("  → Inverse Brier Score by laminations...\n")
  plot_results(
    post_training_data_summarised,
    y     = "Inverse Brier Score",
    colour  = "laminations",
  ...
  cat("  → Proportion Correct by laminations...\n")
  plot_results(
    post_training_data_summarised,
    y     = "Proportion Correct",
    colour  = "laminations",
  ...
  cat("  → Brier Skill Score by layers...\n")
  plot_results(
  ...
  cat("  → Inverse Brier Score by layers...\n")
  plot_results(
    dplyr::mutate(post_training_data_summarised, layers = factor(layers, levels = c(1:3))),
    y     = "Inverse Brier Score",
    colour  = "layers",
  ...
  cat("  → Proportion Correct by layers...\n")
  plot_results(
    dplyr::mutate(post_training_data_summarised, layers = factor(layers, levels = c(1:3))),
    y     = "Proportion Correct",
    colour  = "layers",
...
if (rerun_analysis) {
  ...
  cat("Reformatting dataset and applying Smithson-Verkuilen shrinkage...\n")
  ...
  compute_cv_block_parallel <- function(df, formulas, response_name,
    ...
    }, .options = furrr::furrr_options(seed = 1), .progress = FALSE)
  ...
  cat("Fitting candidate models with logit link...\n")
  cat("  → Fitting Inverse Brier Score models...\n")
  ...
  cat("  → Fitting Proportion Correct models...\n")
  ...
  cat("Computing likelihood ratio tests...\n")
  ...
  cat("Comparing link functions (logit vs log‑log)...\n")
  ...
  cat("Fitting dispersion model variants...\n")
  cat("  → Fitting models without phi sub-model...\n")
  ...
  cat("Checking residuals and heteroskedasticity...\n")
  ...
  cat("Computing estimated marginal means (EMMs)...\n")

  # Recurrence Grammar Type interaction

  cat("  → Recurrence × Grammar Type (Inverse Brier)...\n")
  ...
  cat("  → Plotting Recurrence × Grammar Type EMMs (Inverse Brier)...\n")
  ...
  cat(emm_lams_gram_inverse_brier_kable, sep = "\n\n")
  ...
  cat("Analysis complete! Report saved to results/regression_output.md\n")
}